...
export async function login(formData: FormData): Promise<AuthResult> {
  ...
  // Return success without forcing a redirect
  return { success: true }
}
...
export async function signup(formData: FormData): Promise<AuthResult> {
  ...
  // If signup was successful and user was created (not just confirmation email sent)
  if (authData.user) {
    try {
      // Create a profile for the user immediately with first and last name
      await supabase
        .from('profiles')
        .insert([{
...
// GitHub Sign In
export async function signInWithGithub(): Promise<OAuthAuthResult> {
  const supabase = await createClient();

...
import { fetchLinkedInProfileByUrl } from '@/utils/api/linkedin';
...
/**
 * Imports profile data from LinkedIn by URL
 */
export async function importLinkedInProfile(url: string) {
  'use server';
  
  try {
    const profileData = await fetchLinkedInProfileByUrl(url);

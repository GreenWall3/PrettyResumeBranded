...
/**
 * Checks if the authenticated user has an active Pro subscription
 * @returns {Promise<boolean>} True if the user is on a Pro plan, false otherwise
 */
export async function isUserOnProPlan(): Promise<boolean> {
  try {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
    ...
    // Check if the user has a Pro plan and it's not canceled
    return data?.subscription_plan?.toLowerCase() === 'pro' && 
           data?.subscription_status?.toLowerCase() !== 'canceled';
  } catch (error) {

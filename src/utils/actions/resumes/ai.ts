...
import { getSubscriptionPlan, isUserOnProPlan } from "@/utils/actions/stripe/actions";
...
export async function convertTextToResume(prompt: string, existingResume: Resume, targetRole: string, config?: AIConfig) {
  ...
  const isProUser = await isUserOnProPlan();
  
  // Initialize AI client based on user status and config

...
export async function createBaseResume(
  ...
  const newResume: Partial<Resume> = {
    ...
    // Personal Information - use profile fallback only for 'import-profile' option
    first_name: importOption === 'import-profile' ? (profile?.first_name || '') : selectedContent?.first_name || '',
    last_name: importOption === 'import-profile' ? (profile?.last_name || '') : selectedContent?.last_name || '',
    email: importOption === 'import-profile' ? (profile?.email || '') : selectedContent?.email || '',
    phone_number: importOption === 'import-profile' ? (profile?.phone_number || '') : selectedContent?.phone_number || '',
    location: importOption === 'import-profile' ? (profile?.location || '') : selectedContent?.location || '',
    
    // URLs - use profile fallback only for 'import-profile' option
    website: importOption === 'import-profile' ? (profile?.website || '') : selectedContent?.website || '',
    linkedin_url: importOption === 'import-profile' ? (profile?.linkedin_url || '') : selectedContent?.linkedin_url || '',
    github_url: importOption === 'import-profile' ? (profile?.github_url || '') : selectedContent?.github_url || '',

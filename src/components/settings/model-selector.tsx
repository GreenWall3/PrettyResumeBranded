...
export function ModelSelector() {
    ...
    if (storedKeys) {
      try {
        ...
        if (storedModel) {
          ...
          // For Gemini models, always allow selection regardless of API keys
          if (model?.provider === 'google' || (model && keys.some((k: ApiKey) => k.service === model.provider))) {
            setDefaultModel(storedModel)
          } else {
            // Default to Gemini if previous model is no longer available
            setDefaultModel('gemini-2.0-flash')
            localStorage.setItem(MODEL_STORAGE_KEY, 'gemini-2.0-flash')
            toast.info(`Switched to Gemini 2.0 as previous model was unavailable`)
          }
        } else {
          // No stored model, set to Gemini by default
          setDefaultModel('gemini-2.0-flash')
          localStorage.setItem(MODEL_STORAGE_KEY, 'gemini-2.0-flash')
        }
      } catch (error) {
        console.error('Error loading API keys:', error)
        // Set default to Gemini even if there's an error
        setDefaultModel('gemini-2.0-flash')
        localStorage.setItem(MODEL_STORAGE_KEY, 'gemini-2.0-flash')
      }
    } else {
      // No API keys stored, set default to Gemini
      setDefaultModel('gemini-2.0-flash')
      localStorage.setItem(MODEL_STORAGE_KEY, 'gemini-2.0-flash')
    }
  }, [])
  ...
  const isModelSelectable = (modelId: string) => {
    ...
    // Always allow Google (Gemini) models regardless of user-provided API keys

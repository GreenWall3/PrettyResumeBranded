...
const AI_MODELS: AIModel[] = [
  // Stable models - Move Gemini to top and mark as stable (not unstable)
  { id: 'gemini-2.0-flash', name: 'Gemini 2.0 Flash (Recommended)', provider: 'google', unstable: false },
  { id: 'gemini-2.0-flash-lite-preview-02-05', name: 'Gemini 2.0 Flash Lite', provider: 'google', unstable: false },
...
export function ApiKeysForm({ isProPlan }: { isProPlan: boolean }) {
  ...
  useEffect(() => {
    ...
    if (storedModel) {
      ...
      // If no stored model, set Gemini as default (both for pro and non-pro users)
  ...
  const handleUpdateKey = (service: ServiceName) => {
    ...
    const autoSelectModel = () => {
      switch (service) {
        ...
        default:
          return defaultModel || 'gemini-2.0-flash'
      }
    }

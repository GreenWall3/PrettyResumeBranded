          ...
          <CardContent className="p-3 sm:p-4 space-y-3 sm:space-y-4">
            <div className="space-y-2 sm:space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Tiptap
                  content={(edu.achievements || []).length > 0 
                    ? `<ul>${(edu.achievements || []).map(achievement => `<li>${achievement}</li>`).join('')}</ul>`
                    : ''}
                  onChange={(newContent) => {
                    ...
                    // If no list items but there's content, try to extract from plain text with line breaks
                    const achievements = listItems.length > 0 
                      ? listItems 
                      : newContent
                          .replace(/<[^>]*>/g, '') // Remove any HTML tags
                          .split(/\n|<br\s*\/?>/) // Split by newlines or <br> tags
                          .map(line => line.replace(/^[•\-\*]\s*/, '').trim()) // Remove bullet markers
                          .filter(Boolean); // Remove empty lines
                    
                    updateEducation(index, 'achievements', achievements as string[]);
                  }}
                  editorProps={{
                    attributes: {
                    ...
                    handleKeyDown: (view: EditorView, event: KeyboardEvent) => {
                      if (event.key === 'Enter' && !event.shiftKey) {
                        // Let TipTap handle Enter key for list items
                        return false;
                      }
                      return false;
                    }

...
import { Plus, Trash2, GripVertical, Loader2, Sparkles, Check, X, Bold } from "lucide-react";
...
// Let's modify the toggle bold function to use our editor registry
const toggleBoldInEditor = (projectIndex: number, pointIndex: number) => {
  // Create the editor ID that matches what we set in the Tiptap component
  const editorId = `project-${projectIndex}-point-${pointIndex}`;
  
  // Get the editor from our global registry
  if (typeof window !== 'undefined') {
    const editors = (window as any).__TIPTAP_EDITORS__;
    
    if (editors && editors[editorId]) {
      // Use the editor's toggleBold command
      editors[editorId].chain().focus().toggleBold().run();
    }
  }
};
...
export const ProjectsForm = memo(function ProjectsFormComponent({
  ...
  const [hiddenPoints, setHiddenPoints] = useState<{ [key: number]: { [key: number]: boolean } }>({});
  const [newPointAdded, setNewPointAdded] = useState<{projectIndex: number, pointIndex: number} | null>(null);
  ...
  // Effect to focus on newly added point
  useEffect(() => {
    if (newPointAdded) {
      const {projectIndex, pointIndex} = newPointAdded;
      const editorElement = document.querySelector(
        `[data-project="${projectIndex}"][data-point="${pointIndex}"] .ProseMirror`
      );
  ...
  const handleTechKeyPress = (e: ReactKeyboardEvent<HTMLInputElement>, projectIndex: number) => {
    if (e.key === 'Enter') {
      e.preventDefault();
  ...
  const hideDescriptionPoint = (projectIndex: number, descIndex: number) => {
  ...
  const restoreDescriptionPoint = (projectIndex: number, descIndex: number) => {
    setHiddenPoints(prev => {
      const updatedHidden = { ...prev };
      if (updatedHidden[projectIndex]) {
        const projectPoints = { ...updatedHidden[projectIndex] };
        delete projectPoints[descIndex];
        
        if (Object.keys(projectPoints).length === 0) {
          delete updatedHidden[projectIndex];
        } else {
          updatedHidden[projectIndex] = projectPoints;
        }
      }
  ...
  const removeDescriptionPoint = (projectIndex: number, descIndex: number) => {
    const updated = [...projects];
    updated[projectIndex].description = updated[projectIndex].description.filter((_, i) => i !== descIndex);
    onChange(updated);
    
    // Also remove from hidden state
    restoreDescriptionPoint(projectIndex, descIndex);
  };

  // Function to add a new point to a project
  const addProjectPoint = (projectIndex: number) => {
    const updated = [...projects];
    const newPointIndex = updated[projectIndex].description.length;
    updated[projectIndex].description = [...updated[projectIndex].description, ""];
    onChange(updated);
    
    // Force a re-render and focus by updating state
    setTimeout(() => {
      setNewPointAdded({
        projectIndex,
                ...
                <div className="space-y-2 sm:space-y-3">
                  ...
                  <div className="space-y-2 pl-0">
                    {project.description.map((desc, descIndex) => {
                      if (hiddenPoints[index]?.[descIndex]) {
                        return (
                          <div key={descIndex} className="flex gap-1 items-center p-2 bg-gray-50/80 rounded-lg">
                            <div className="flex-1 text-xs text-gray-500 italic">
                              Point hidden - click to restore or delete permanently
                            </div>
                            <div className="flex gap-1">
                              <Button
                                variant="ghost"
                                size="sm"
                                onClick={() => restoreDescriptionPoint(index, descIndex)}
                                className="h-7 px-2 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50/60"
                              >
                              ...
                              <Button
                                ...
                                className="h-7 px-2 text-red-600 hover:text-red-700 hover:bg-red-50/60"
                              >
                      ...
                      return (
                        <div key={descIndex} className="flex gap-1 items-start group/item">
                          <div className="flex-1 relative" data-project={index} data-point={descIndex}>
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => toggleBoldInEditor(index, descIndex)}
                              className={cn(
                                "absolute top-1 right-1 z-10",
                                "p-0 opacity-50 group-hover/item:opacity-100",
                                "h-4 w-4 rounded-md",
                            ...
                            <Tiptap
                              content={desc} 
                              editorId={`project-${index}-point-${descIndex}`}
                              onChange={(newContent) => {
                                const updated = [...projects];
                              ...
                              className={cn(
                                ...
                                newPointAdded?.projectIndex === index && newPointAdded?.pointIndex === descIndex && [
                                  "animate-pulse border-2 border-violet-400",
                                  "bg-violet-50/80",
                                  "shadow-[0_0_15px_-3px_rgba(139,92,246,0.3)]",
                                ]
                              )}
                            />
                            ...
                            {improvedPoints[index]?.[descIndex] ? (
                              ...
                              <>
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  onClick={() => hideDescriptionPoint(index, descIndex)}

...
import { Plus, Trash2, GripVertical, Check, X, Loader2, Sparkles, Bold } from "lucide-react";
...
export const WorkExperienceForm = memo(function WorkExperienceFormComponent({ 
  ...
  useEffect(() => {
    if (newPointAdded) {
      ...
      const editorElement = document.querySelector(
        `[data-experience="${projectIndex}"][data-point="${pointIndex}"] .ProseMirror`
      );
  ...
  const hideDescriptionPoint = (expIndex: number, descIndex: number) => {
  ...
  const restoreDescriptionPoint = (expIndex: number, descIndex: number) => {
    setHiddenPoints(prev => {
      const updatedHidden = { ...prev };
      if (updatedHidden[expIndex]) {
        const expPoints = { ...updatedHidden[expIndex] };
        delete expPoints[descIndex];
        
        if (Object.keys(expPoints).length === 0) {
          delete updatedHidden[expIndex];
        } else {
          updatedHidden[expIndex] = expPoints;
        }
      }
  ...
  const removeDescriptionPoint = (expIndex: number, descIndex: number) => {
    ...
    restoreDescriptionPoint(expIndex, descIndex);
  };

  // Function to add a new point to an experience
  const addExperiencePoint = (expIndex: number) => {
    const updated = [...experiences];
    const newPointIndex = updated[expIndex].description.length;
    ...
    setTimeout(() => {
      setNewPointAdded({
        projectIndex: expIndex,
        pointIndex: newPointIndex
      });
    }, 10);
  };

  // Add this function for toggling bold in the editor
  const toggleBoldInEditor = (expIndex: number, pointIndex: number) => {
    // Create the editor ID that matches what we set in the Tiptap component
    const editorId = `experience-${expIndex}-point-${pointIndex}`;
                    ...
                    {exp.description.map((desc, descIndex) => {
                      ...
                      return (
                        <div key={descIndex} className="flex gap-1 items-start group/item">
                          <div className="flex-1 relative" data-experience={index} data-point={descIndex}>
                            <Button
                              variant="ghost"
                              size="icon"
                            ...
                            <Tiptap
                              content={desc} 
                              editorId={`experience-${index}-point-${descIndex}`}
                              onChange={(newContent) => {
                                const updated = [...experiences];
                              ...
                              className={cn(
                                ...
                                newPointAdded?.projectIndex === index && newPointAdded?.pointIndex === descIndex && [
                                  "animate-pulse border-2 border-cyan-400",
                                  "bg-cyan-50/80",
